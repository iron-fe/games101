                                      ...
                                      float zNear, float zFar)
{
    ...
    float cot_val = tan(MY_PI/(2-(radian/2)));
    frustum <<  cot_val/aspect_ratio, 0, 0, 0, 
                0, cot_val, 0, 0, 
...
Eigen::Matrix4f get_rotation(Vector3f axis, float angle)
{
    ...
    model = cos(radian) * I + (1 - cos(radian)) * mat * mat.transpose() + sin(radian) * N;
    model(3,3) = 1;
    return model;
}
